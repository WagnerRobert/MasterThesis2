...
for location in counts:
    ...
    while keepGoing:
        ...
        for subsetA in sorted(counts[location], key= lambda x : len(counts[location]), reverse=True):
            if subsetA in workedList:
                continue

            for subsetB in sorted(counts[location], key= lambda x : len(counts[location])):
