...
def create_plot(query_protein_sequence, pro_matches, entry, numProfileProteins, resultfile_info, paths, prosite, patternMatches, top_matches, complete_matches):
    ...
    for start, end in prosite:
        rect = plt.Rectangle((start - 0.5, 0), end-start + 1, 99, facecolor="#0000FF")
        rect.set_alpha(0.5)
        plt.gca().add_patch(rect)

    for start, end in patternMatches:
        rect = plt.Rectangle((start - 0.5, 0), end-start + 1, 99, facecolor="#00FF00")
        rect.set_alpha(0.5)
        plt.gca().add_patch(rect)
    ...
    masterthesis2.evaluation.doSegmentEvaluation(patternMatches,zscore_count, 0.5 )
