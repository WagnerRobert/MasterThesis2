...
def eval_segment(query_protein_sequence, pro_matches, numProfileProteins, patternMatches, top_matches, cutoff):
...
def eval_aminoacid(query_protein_sequence, pro_matches, numProfileProteins, patternMatches, top_matches, cutoff):
    name = query_protein_sequence[0]
    #print name
    sequence = query_protein_sequence[1]
    #print sequence
    ...
    if len(patternMatches) == 0:
        print "protein has no nlsDB motif matches and is therefore not considered for the evaluation"
        return None
    precision, recall = masterthesis2.evaluation.doEvaluation(patternMatches,zscore_count, cutoff)
